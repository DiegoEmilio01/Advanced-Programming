...
def desencriptar(funcion_decodificadora, tipo_archivo):
    ...
    def decorador(funcion_a_decorar):
        def wrapper(*args, **kwargs):
            for elemento in funcion_a_decorar(*args, **kwargs):
                #print(elemento)
                if tipo_archivo == "canciones":
                    yield Cancion(decodificar(elemento[0]), decodificar(elemento[1]), decodificar(elemento[2]), decodificar(elemento[3]))
                elif tipo_archivo == "artistas":
                    yield Artista(decodificar(elemento[0]), decodificar(elemento[1]), decodificar(elemento[2]), decodificar(elemento[3]))
                elif tipo_archivo == "usuarios":
                    yield Usuario(decodificar(elemento[0]), decodificar(elemento[1]), decodificar(elemento[2]))
                elif tipo_archivo == "ratings":
                    yield Rating(decodificar(elemento[0]), decodificar(elemento[1]), decodificar(elemento[2]))
