...
def usuarios_por_antiguedad(usuarios):
    usuarios_ordenados = sorted(list(usuarios), key=lambda usuario : datetime.strptime(usuario.fecha_ingreso, "%d/%m/%Y"))
    for i in usuarios_ordenados:
        yield i


def ratings_usuarios(username, ratings):
    usuarios_ordenados = sorted(list(usuarios), key=lambda usuario : usuario.rating)
    for i in usuarios_ordenados:
        yield i
